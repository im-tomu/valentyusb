...
class USBWishboneBridge(Module):
    ...
    def __init__(self, usb_core, clk_freq=12000000):
        ...
        self.sync += [
            ...
            If(rx_data_ce,
                data.eq(Cat(usb_core.data_recv_payload, data[0:24]))
            ).Elif(tx_data_ce,
                data.eq(self.wishbone.dat_r)
            )
        ]
        ...
        # ]
        fsm.act("IDLE",
            NextValue(self.debug_in_progress, 0),
            If(usb_core.data_recv_put,
                If(usb_core.tok == PID.SETUP,
                    ...
                    If(usb_core.data_recv_payload[0:7] == 0x40,
                        NextValue(self.debug_in_progress, 1),
                        cmd_ce.eq(1),
        ...
        fsm.act("WRITE_DATA",
            ...
            If(self.wishbone.ack,
                NextState("WAIT_USB_OUT_ACK")
            )
        )
        fsm.act("WAIT_USB_OUT_ACK",
            self.sink_valid.eq(1),
            If(usb_core.start,
                NextState("WAIT_USB_OUT_ACK_END"),
            )
        )
        fsm.act("WAIT_USB_OUT_ACK_END",
            If(usb_core.end,
                NextState("WAIT_USB_IN_ACK")
            )
        )
        fsm.act("WAIT_USB_IN_ACK",
            self.sink_valid.eq(1),
            If(usb_core.start,
                NextState("WAIT_PKT_END")
            )
        )
        fsm.act("WAIT_USB_ACK",
            self.sink_valid.eq(1),
            If(usb_core.start,
                NextState("WAIT_PKT_END")
            )
